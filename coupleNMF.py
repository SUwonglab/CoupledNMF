...
parser.add_argument('-lambda1', dest='lambda1', type=float,
                    help='lambda1, hyperparameters to control the term NMF for E')
...
for i in range(K):
    ...
    for j in range(len(p)):
        fout3.write("cluster " + str(i + 1) + ": " + E_symbol[p[j]] + "\t" + P_symbol[q[j]] + "\t" + str(
            E_p_final[i, p[j]]) + "\t" + str(P_p_final[i, q[j]]) + "\n")
